...
def part_1(sheet: str) -> int:
    """Calculates checksum"""
    numbers: Iterable[List[str]] = process_sheet(sheet)
    checksum: int = 0
    for number_row in numbers:
        ints: List[int] = list(map(int, number_row))
        max_num: int = max(ints)
        min_num: int = min(ints)
        checksum += (max_num - min_num)

    return checksum


def part_2(sheet: str) -> int:
    """Calculates even distribution"""
    numbers: Iterable[List[str]] = process_sheet(sheet)
    checksum: int = 0
    for number_row in numbers:
        row: List[int] = list(map(int, number_row))
        for i in row:
            other_nums = [x for x in row if x != i]
            filtered: List[int] = list(filter(lambda x: x % i == 0, other_nums))
            if any(filtered):
                checksum += filtered[0] // i

    return checksum


def process_sheet(sheet: str) -> Iterable[List[str]]:
    """Process a sheet into a map object, with a list of strings"""
    lines: List[str] = sheet.strip().split("\n")
...
"""
    print(part_1(DAY2_INPUT))
    print(part_2(DAY2_INPUT))
